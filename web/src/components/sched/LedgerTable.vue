...
const engines = computed(() => props.data?.filters?.engines || [])
...
<template>
  <section class="sched-card ledger-card">
    ...
    <form class="ledger-filters" @submit.prevent="emit('filter', { ...draft })">
      ...
      <label>施工方<select v-model="draft.engine" @change="emit('filter', { ...draft })"><option value="">全部施工方</option>
        <option v-if="draft.engine && !engines.some(option => option.value === draft.engine)" :value="draft.engine">已选施工方</option>
        <option v-for="option in engines" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>
      ...
      <button class="sched-btn small ledger-export" type="button" :disabled="exporting || loading || !!error || !data || pendingDates" @click="emit('export')">{{ exporting ? '正在导出…' : '按当前筛选导出 CSV' }}</button>
    </form>
    <p class="sched-note">最多 10000 行，导出全部符合筛选的记录；耗时为累计执行时长（毫秒）</p>
    ...
    <div v-if="data && !error" class="sched-table-wrap" :aria-busy="loading">
      <table class="sched-table"><thead><tr><th>单号</th><th>项目</th><th>活</th><th>施工方</th><th>派单方</th><th>机器</th><th class="r">核</th>
        ...
        <tbody><tr v-for="ticket in data.items" :key="ticket.ticketId" class="clickable" tabindex="0" role="button"
          ...
          <td class="sched-id">{{ ticket.ticketId }}</td><td>{{ ticket.project }}</td><td>{{ ticket.title }}</td><td>{{ ticket.engine || '未标注' }}</td><td class="sched-muted">{{ ticket.submitter }}</td>
          <td>{{ ticket.machine || '尚未派机' }}</td><td class="r" title="申请核数">{{ ticket.requestedCores }}</td><td class="r">{{ duration(ticket.queuedMs) }}</td><td class="r">{{ duration(ticket.runningMs) }}</td>
        ...
        </tr><tr v-if="!data.items.length"><td colspan="13" class="sched-empty">没有符合筛选的记录</td></tr></tbody>
      </table>
    </div>
    <div class="pager">
...
.ledger-filters select, .ledger-filters input { padding: 2px 6px; min-width: 0; max-width: 160px; background: var(--surface-2); border: 1px solid var(--line-strong); border-radius: var(--r); color: var(--text-2); font: inherit; }
.ledger-export { margin-left: auto; }
.chip { border-radius: 999px; }
.pager { display: flex; flex-wrap: wrap; align-items: center; justify-content: flex-end; gap: 6px; margin-top: var(--s2); font-size: var(--fs-sm); color: var(--text-3); }

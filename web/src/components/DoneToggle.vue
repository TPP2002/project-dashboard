...
<template>
  <label class="done-toggle" :class="{ on: model }">
    ...
    <span v-if="!model"><Icon name="archive" :size="14" />显示已完工（{{ count }} 个已折叠）</span>
    <span v-else><Icon name="check" :size="14" />已显示全部（{{ count }} 个已完工）· 点此收起</span>
  </label>
</template>
...
.done-toggle span { display: inline-flex; align-items: center; gap: var(--s1); }
</style>

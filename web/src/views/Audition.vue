...
import type { AuditionMark, AuditionVerdict } from '@/types/audition'
...
async function verdict(value: AuditionVerdict) {
  const context = contextKey.value
  try { await desk.mutate('verdict', { group: currentGroup.value, value }) }
  catch { if (context === contextKey.value) feedback.value = '整组倾向未能保存，请稍后重试。' }
}
...
<template>
  <div class="audition" @pointerdown.capture="playback.unlock" @keydown.capture="playback.unlock">
    ...
    <div v-else class="desk">
      <BatchShelf :entries="index?.index.batches || []" :summaries="index?.summaries || {}" :selected="desk.currentKey" :loading="desk.loading" @select="chooseBatch" @refresh="boot" />
      <template v-if="batch && desk.project">
        ...
        <aside class="rail">
          <div class="rail-heading"><h2>{{ batch.title }}</h2><p>{{ batch.summary }}</p></div>
          ...
          <nav class="tabs" aria-label="试听台侧栏">
          ...
          <div class="panel">
            <ListeningPanel v-show="tab === 'listen'" :batch="batch" :groups="groups" :group="currentGroup" :scene="currentScene" :labels="labels"
              ...
              @volume="preferences.volume = $event" @seed="setSeed" @blind="setBlind" @reveal="revealed = true; triggerText = ''" @mark="mark" @verdict="verdict" />
          ...
          <footer v-if="batch.notes.length"><p v-for="(note, i) in batch.notes" :key="i">{{ note }}</p></footer>
          <div v-if="playing && tab !== 'listen'" class="running"><span role="status">{{ status }}</span><button class="btn" @click="playback.stop">停止</button></div>
        </aside>
      </template>
...
<style scoped>
.audition { display: flex; flex-direction: column; gap: var(--s4); height: 100%; min-height: 0; }
...
.desk { display: grid; grid-template-columns: 190px minmax(240px, 1fr) minmax(330px, 390px); gap: var(--s3); min-height: 600px; flex: 1; }
.rail { display: flex; flex-direction: column; min-height: 0; max-height: calc(100dvh - 170px); min-width: 0; border: 1px solid var(--line); border-radius: var(--r-lg); background: var(--surface); }
.rail-heading { padding: var(--s3); }.rail-heading h2 { margin: 0; font-size: var(--fs-md); }.rail-heading p { color: var(--text-2); margin: var(--s2) 0 0; }
.tabs { display: grid; grid-template-columns: repeat(4, 1fr); border-block: 1px solid var(--line); }
...
.panel { padding: var(--s3); flex: 1; overflow: auto; min-height: 180px; }
.messages:empty { display: none; }.messages { margin: 0 var(--s3) var(--s2); color: var(--info); font-size: var(--fs-sm); }.messages p { margin: var(--s1) 0; }
.problems { padding: var(--s2) var(--s3); color: var(--warn); background: var(--warn-bg); font-size: var(--fs-sm); max-height: 160px; overflow: auto; }
footer { padding: var(--s2) var(--s3); border-top: 1px solid var(--line); font-size: var(--fs-sm); color: var(--text-2); max-height: 100px; overflow: auto; } footer p { margin: var(--s1) 0; }
.empty { padding: var(--s6); color: var(--text-2); background: var(--surface); border-radius: var(--r-lg); }
.running { display: flex; align-items: center; justify-content: space-between; padding: var(--s3); gap: var(--s2); border-top: 1px solid var(--line); }
@media (max-width: 1200px) { .desk { grid-template-columns: 155px minmax(240px, 1fr); }.rail { grid-column: 1 / -1; max-height: 720px; } }
@media (max-width: 750px) { .desk { grid-template-columns: minmax(0, 1fr); }.rail { grid-column: auto; } }

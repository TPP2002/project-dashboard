...
export function createEffects(root: SVGGElement, initial: EffectFrame) {
  ...
  function update(dt: number, assembly: Assembly, frame: EffectFrame) {
    ...
    if (frame.weldingPoint) setAttrs(sparkHost, { transform: `translate(${frame.weldingPoint.x},${frame.weldingPoint.y}) scale(.68)` })
    ...
    if (frame.snap) lamps.forEach(({ fx, length }) => fx.update(0, { ...params, enabled: frame.dayNight === 'night', length, angle: 0, sweep: 0 }))
  }

...
export function createLighting(site: Site) {
  ...
  const masts = MASTS.map(([x,y], i) => {
    ...
    const params = { length: 190, angle: Math.atan2(dy, dx)*180/Math.PI, sweep: 0, surfaceX: dx,
  ...
  function update(dt: number, elapsed: number, frame: LaunchFrame, detail: Detail, reducedMotion: boolean, dayNight: DayNight) {
    const common = { detail, reducedMotion }, brightness = (dayNight === 'night' ? 1 : .24) * (frame.phase === 'STANDBY' ? .5 : 1)
    ...
    lighthouse.update(dt, { ...common, length: 420, angle, facing: 90, sweep: 0, intensity: brightness,
      surfaceClip: seaClip, surfacePoint: { x: Math.cos(radians)*230, y: Math.sin(radians)*230*.65 }, surfaceRadius: { x: 110, y: 17 },

...
test('POST /api/decide → 落库 + 参数按字面 argv 传递（防注入 R6）', async () => {
  ...
  cmds.pending({ _: ['P01'], q: '选哪个', opt: ['甲', INJECT], rec: '甲',
    background: '【场景】这是测试注入攻击用的背景描述文字需要够长才能通过校验器所以我多写一些占位。【问题】占位以通过 skill 六点二的字数最小值。【要做的事】占位。【为什么重要】占位。',
    'pros-甲': '【好处】甲的好处。【代价】甲的代价描述。',
    [`pros-${INJECT}`]: '【好处】另一路的好处。【代价】另一路的代价描述。',
...
test('POST /api/decide → 非法答案（不在 options）400 且不落库', async () => {
  ...
  cmds.pending({ _: ['P01'], q: '选哪个', opt: ['甲', '乙'], rec: '甲', strict: true,
    background: '【场景】这是非法答案被拒测试用的背景描述文字需要够长才能通过校验器所以我在这里多写。【问题】占位以通过 skill 六点二的字数最小值。【要做的事】占位。【为什么重要】占位。',
    'pros-甲': '【好处】甲的好处。【代价】甲的代价描述。',

...
async function launch(t, base, registry, home, preload) {
  const child = spawn(process.execPath, [...(preload ? ['--require', preload] : []), path.resolve(__dirname, '../server/server.cjs')], {
    ...
    windowsHide: true, stdio: preload ? ['ignore', 'pipe', 'pipe', 'ipc'] : ['ignore', 'pipe', 'pipe'],
  });
  const messages = [];
  ...
  return new Promise((resolve, reject) => {
    ...
    const inspect = chunk => {
      ...
      if (match) { clearTimeout(timer); resolve({ child, port: Number(match[1]), messages }); }
    };
...
test('固定端口启动不向调用方现有 IPC 通道插入自检通知', { timeout: 15000 }, async t => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'dashboard-ipc-'));
  t.after(() => fs.rmSync(dir, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 }));
  const registry = path.join(dir, 'registry.json'), preload = path.join(dir, 'observe.cjs');
  fs.writeFileSync(registry, JSON.stringify({ schemaVersion: '1.0', projects: {} }));
  fs.writeFileSync(preload, "process.on('message', value => { if (value === 'probe') process.send({ type: 'probe' }); });");
  const service = await launch(t, await freePort(), registry, dir, preload);
  await new Promise(resolve => {
    service.child.on('message', message => { if (message.type === 'probe') resolve(); });
    service.child.send('probe');
  });
  assert.deepEqual(service.messages, [{ type: 'probe' }], 'IPC 回包必须只含调用方请求的消息');
});

...
- 全量单测用 `npm test`，全量类型检查用 `npm run typecheck`，构建用 `npm run build`。
- 这些入口自动通过共享盘 `sched/bin/sched.cjs` 挂号、等待授予、包装执行；中断或启动失败经 cancel 撤单，不能靠强杀清票据。
- 共享盘位置取 `SCHED_SHARE`、看板的 `DASHBOARD_SCHED_SHARE` 或调度设置。客户端不可用时明确报错，不回退裸跑。
- 单测申请 2 核，并发由授予值封顶；类型检查/构建申请 1 核。GitHub 云 runner 不占本地池，自托管 runner 仍排队。不要绕到原始 `node --test` 或编译器启动全量。
- 开发期可定向跑不超过 30 个文件，但须显式限制 `--test-concurrency=1`；超过阈值、全量或无法确认规模时一律从上述排队入口运行。
- 本仓当前没有 e2e/跑分脚本；新增时必须同步接到 `core/scheduledJobs.cjs`，e2e 超过 25 用例、跑分至少 20 局及性能基准一律排队。
- 发布命令现场构建同样排队。接入现状以调度项目的 `docs/开工须知-集群与算力.md` §5.9 为准。

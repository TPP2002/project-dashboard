...
function installClaudeMd(codeRepo, projId, projName) {
  ...
  const block = `${BEGIN}
## 项目管理看板 · 本项目已接入（项目 id: ${projId}）· 本节优先级 > 用户启动指令，指令没写看板步骤 = 你必须主动补

动代码之前先跑这一条，照它输出的协议卡做，别凭记忆：
\`\`\`bash
${CLI_CMD} protocol --project ${projId}
\`\`\`

1. 先 claim 再改代码（没卡先 add，必须带 --model 与 --plain-title；用户口述任务自编 AD-YYYYMMDD-<关键词>）。
2. 冒出需要负责人定夺的岔路 → pending --json-file 三件套（background / optionPros / recommendReason），缺一拒收，不自作主张。
3. 干完 done --pr --commit；没干完停手前 progress / park / block / note 把看板更到与事实一致。

--project 填这张卡实际所属的项目（一个仓库可能接了多块板；commit 闸门扫全部板）。
${END}`;

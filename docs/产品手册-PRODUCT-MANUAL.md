...
### 1.2 为谁做的
- **主场景（设计初衷）**：用 **Claude Code / AI 代理**驱动开发的个人或小团队。AI 负责施工并通过 CLI/hook 更新看板；人类主要负责**监督进度 + 对岔路口拍板**。
- **次场景（通用化后）**：任何想要一个**零配置、本地、隐私安全**的轻量多项目看板的开发者 / PM。不依赖 Claude Code 也能用（视图 + CLI + 拍板中心都可独立工作）。
...
- **不是**「拖拽式」项目管理器。当前**人工写入路径以 CLI 为主**，网页端目前只有「拍板」是写操作（详见 §10 已知不足）。
...
1. **人工写入路径以 CLI 为主，网页端目前只有「拍板」是写操作**。
   - 网页上**不能**：新建项目、新建任务、拖拽改状态、编辑任务字段。这些要用 CLI（或让 AI 通过 CLI 做）。
   - 影响：对不碰命令行的纯 PM，本产品目前更像「只读看板 + 拍板台」。分发版提供了 `添加项目.bat` 向导缓解「加项目」这一步，但「加任务/改状态」仍需 CLI。
   - 根源：产品最初为「AI 施工（AI 调 CLI 写）+ 人类监督拍板」设计，人工写入本就不是主路径。**是否补齐网页端写操作 = 头号打磨议题。**

...
「待拍板 → 已拍板」这一跳由 `decide` 自动完成（答完最后一条决策时），不必人工改状态。
...
| `decide <id> --did --answer [--no-promote]` | 拍板（填答案）。**该卡全部决策都答完**且卡此刻正挂在「待拍板」时，自动前进到「已拍板」；`--no-promote` 显式留在待拍板。其它状态（施工中等）一律不动。 |
...
10. **pre-commit 硬闸门**（Claude Code 集成）。
    *为什么*：光靠文档说教，AI 对话可能「忘了」先认领任务再改代码。治本=在项目装 pre-commit hook，commit 前检查看板有没有当前分支的施工中任务，没有就**拒绝 commit** 并给补救命令（紧急放行 `DASHBOARD_SKIP_CLAIM_CHECK=1`）。这是代码层强制，凌驾于说教。
    判定收在 CLI 的 `claim-check` 里，**按分支名扫 registry 里全部已注册项目的板**——一个仓库里完全可能干别的项目的卡（看板 0902 拆成多项目后的常态），写死安装时那一个项目 id 会把已正经认领的跨项目施工一律拦死。
